...
class Fluid(object):
    ...
    def display(self, complex=False, u_e=None):
        ...
        if not np.any(u_e)==None:
            u -= u_e
        p=plt.imshow(u, extent=extent, cmap="RdBu_r")
        plt.colorbar(p)
    ...
    def run_live(self, stop, every=100):
        ...
        while(self.time<=stop):
            ...
            if(self.it % every == 0):
                ...
                print("Iteration ilya \t %d, time \t %f, time remaining \t %f. TKE: %f" %(self.it,
