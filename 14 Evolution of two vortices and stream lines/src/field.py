...
def VortexPaar(x, y, Re, **kwargs):
    nx, ny = 400, 400
    x = np.linspace(-2, 2, nx)
    y = np.linspace(-2, 2, ny)
    X, Y = np.meshgrid(x, y)

    # Vortex definitions (center, strength, shape, rotation)
    vortices = [
        {"center": (-0.7, 0.0), "strength": 50, "rx": 0.4, "ry": 0.25, "angle": np.pi/6},
        {"center": (0.7, 0.3),  "strength": -50, "rx": 0.2, "ry": 0.6,  "angle": -np.pi/5}
    ]

    def rotated_elliptical_vortex(x0, y0, strength, rx, ry, angle):
        """
        Generates a rotated elliptical Gaussian vortex.
        """
        Xc = X - x0
        Yc = Y - y0
        ...
        r2 = (Xr / rx)**2 + (Yr / ry)**2
        return strength * np.exp(-r2 / 2)

    # Total vorticity field
    omega = np.zeros_like(X)
    for v in vortices:
        x0, y0 = v["center"]
        omega += rotated_elliptical_vortex(x0, y0, v["strength"], v["rx"], v["ry"], v["angle"])

    # Optional smoothing
    field = gaussian_filter(omega, sigma=1.2)
